  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Si è verificato un problema.</p>
        <button onClick={() => { router.invalidate(); reset(); }} className="mt-6 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Riprova</button>
      </div>
    </div>
  );
}
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Portale per la gestione delle minute dei SAL con i fornitori." },
      { property: "og:title", content: "AC SAL Meetings" },
      { name: "twitter:title", content: "AC SAL Meetings" },
      { property: "og:description", content: "Portale per la gestione delle minute dei SAL con i fornitori." },
      { name: "twitter:description", content: "Portale per la gestione delle minute dei SAL con i fornitori." },
